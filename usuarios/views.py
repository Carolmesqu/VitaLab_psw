    ...
    if request.method == "GET":
        ...
        if not senha == confirmar_senha:
            messages.add_message(
                request, constants.ERROR, "As senhas não correspondem."
            )
            return redirect("/usuarios/cadastro")

        if len(senha) < 6:
            messages.add_message(
                request, constants.ERROR, "A senha têm menos de 6 caracteres."
            )
            return redirect("/usuarios/cadastro")

        # Validar se o username do usuário é único
        ...
        try:
            ...
            messages.add_message(
                request,
                constants.ERROR,
                "Error interno do sistema, contate um administrador.",
            )
            return redirect("/usuarios/cadastro")
